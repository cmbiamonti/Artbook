import React from 'react';
import { useForm } from 'react-hook-form';
import { useArtists } from '../../hooks/useArtists';
import { useState, useEffect } from 'react';
import { Loader2, Plus, ChevronDown, ChevronUp } from 'lucide-react';
import { toast } from 'sonner';
import { getCategories, getCategoryByValue } from '../../utils/categories';
import { ImageGallery } from './ImageGallery';
import type { Artwork, CreateArtworkData, UpdateArtworkData } from '../../types/artwork.types';
import { useTranslation } from 'react-i18next';

interface ArtworkFormProps {
  artwork?: Artwork;
  onSubmit: (data: CreateArtworkData | UpdateArtworkData) => Promise<any>;
  isEdit?: boolean;
}

export const ArtworkForm: React.FC<ArtworkFormProps> = ({ 
  artwork, 
  onSubmit,
}) => {
  // ✅ CORRETTO: useTranslation chiamato a livello di componente, non dentro useEffect
  const { t } = useTranslation();

  const { data: artists = [], isLoading: artistsLoading } = useArtists();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [imageFiles, setImageFiles] = useState<File[]>([]);
  
  const [artistInputMode, setArtistInputMode] = useState<'select' | 'input'>('select');
  const [customArtistName, setCustomArtistName] = useState('');
  
  const [sectionsExpanded, setSectionsExpanded] = useState({
    basic: true,
    identification: false,
    logistics: false,
    market: false,
  });

  const {
    register,
    handleSubmit,
    watch,
    setValue,
    formState: { errors },
  } = useForm<CreateArtworkData>({
    defaultValues: artwork ? {
      title: artwork.title,
      artist_id: artwork.artist_id || undefined,
      category: artwork.category,
      subcategory: artwork.subcategory || undefined,
      year: artwork.year || undefined,
      technique: artwork.technique || undefined,
      dimensions: artwork.dimensions || undefined,
      estimated_value: artwork.estimated_value || undefined,
      description: artwork.description || undefined,
      catalog_number: artwork.catalog_number || undefined,
      certificate_authenticity: artwork.certificate_authenticity || undefined,
      certificate_number: artwork.certificate_number || undefined,
      artist_signature: artwork.artist_signature || undefined,
      condition_state: artwork.condition_state || undefined,
      edition_number: artwork.edition_number || undefined,
      current_location: artwork.current_location || undefined,
      location_details: artwork.location_details || undefined,
      insurance_value: artwork.insurance_value || undefined,
      insurance_company: artwork.insurance_company || undefined,
      insurance_expiry: artwork.insurance_expiry || undefined,
      frame_included: artwork.frame_included || undefined,
      frame_description: artwork.frame_description || undefined,
      purchase_price: artwork.purchase_price || undefined,
      purchase_date: artwork.purchase_date || undefined,
      seller_gallery: artwork.seller_gallery || undefined,
      provenance: artwork.provenance || undefined,
      available_for_sale: artwork.available_for_sale || undefined,
      asking_price: artwork.asking_price || undefined,
    } : {
      category: 'dipinti',
      subcategory: undefined,
      certificate_authenticity: undefined,
      artist_signature: undefined,
      condition_state: undefined,
      current_location: undefined,
      frame_included: undefined,
      available_for_sale: undefined,
    },
  });

  const certificateAuth = watch('certificate_authenticity');
  const frameIncluded = watch('frame_included');
  const availableForSale = watch('available_for_sale');
  const selectedCategory = watch('category');

  const categories = getCategories(t);   // ← aggiunta questa riga

  const availableSubcategories = selectedCategory
    ? getCategoryByValue(selectedCategory, t)?.subcategories || []
    : [];

  // ✅ CORRETTO: useEffect senza chiamata a useTranslation al suo interno
  useEffect(() => {
    setValue('subcategory', '');
  }, [selectedCategory, setValue]);

  const toggleSection = (section: keyof typeof sectionsExpanded) => {
    setSectionsExpanded(prev => ({
      ...prev,
      [section]: !prev[section]
    }));
  };

  const handleImagesChange = (files: File[]) => {
    console.log('📸 Images changed:', files.length);
    setImageFiles(files);
  };

  const onFormSubmit = async (data: CreateArtworkData) => {
    try {
      setIsSubmitting(true);

      console.log('═══════════════════════════════════════');
      console.log('📝 FORM SUBMIT');
      console.log('📥 Form data:', data);
      console.log('📸 Image files count:', imageFiles.length);
      console.log('═══════════════════════════════════════');

      if (!window.electronAPI) {
        console.error('❌ electronAPI is not available!');
        // ✅ CORRETTO: usa t() disponibile nel componente
        toast.error(t('errors.electron_api_not_available'));
        setIsSubmitting(false);
        return;
      }

      let finalData: any = { ...data };
      
      if (finalData.year) finalData.year = parseInt(finalData.year as any);
      if (finalData.estimated_value) finalData.estimated_value = parseFloat(finalData.estimated_value as any);
      if (finalData.insurance_value) finalData.insurance_value = parseFloat(finalData.insurance_value as any);
      if (finalData.purchase_price) finalData.purchase_price = parseFloat(finalData.purchase_price as any);
      if (finalData.asking_price) finalData.asking_price = parseFloat(finalData.asking_price as any);

      if (artistInputMode === 'input' && customArtistName.trim()) {
        const artistResult = await window.electronAPI.createArtist({
          name: customArtistName.trim()
        });
        
        if (artistResult.success && artistResult.data) {
          finalData.artist_id = artistResult.data.id;
          // ✅ CORRETTO: usa t() con interpolazione per il nome artista
          toast.success(t('messages.artist_created', { name: customArtistName }));
        } else {
          throw new Error(artistResult.error || 'Failed to create artist');
        }
      }

      delete finalData.image_path;
      delete finalData.imageFile;

      console.log('💾 Final data:', finalData);

      const result = await onSubmit(finalData);

      console.log('📤 onSubmit result:', result);

      if (!result || !result.id) {
        console.error('❌ No artwork ID returned:', result);
        // ✅ CORRETTO: usa t() per il messaggio di errore
        throw new Error(t('components.artworks.artwork_form.artwork_creato_ma_senza'));
      }

      const artworkId = result.id;
      console.log('✅ Artwork created with ID:', artworkId);

      if (!artwork && imageFiles.length > 0) {
        console.log(`📸 Uploading ${imageFiles.length} images...`);
        
        let uploadedCount = 0;
        
        for (let i = 0; i < imageFiles.length; i++) {
          const file = imageFiles[i];
          
          try {
            console.log(`📸 Uploading image ${i + 1}/${imageFiles.length}: ${file.name}`);
            
            const buffer = await file.arrayBuffer();
            console.log(`  📦 Buffer size: ${buffer.byteLength} bytes`);
            
            const saveResult = await window.electronAPI.saveImage(buffer, file.name);
            console.log(`  📤 Save result:`, saveResult);
            
            if (!saveResult.success || !saveResult.data) {
              console.error(`  ❌ Failed to save image ${i + 1}:`, saveResult.error);
              continue;
            }
            
            const savedFilename = saveResult.data;
            console.log(`  ✅ Image saved as: ${savedFilename}`);
            
            const isPrimary = i === 0;
            console.log(`  🔗 Adding to database (primary: ${isPrimary})...`);
            
            const addResult = await window.electronAPI.addArtworkImage(
              artworkId, 
              savedFilename, 
              isPrimary
            );
            
            console.log(`  📤 Add result:`, addResult);
            
            if (addResult.success) {
              uploadedCount++;
              console.log(`  ✅ Image ${i + 1} linked to artwork`);
            } else {
              console.error(`  ❌ Failed to link image ${i + 1}:`, addResult.error);
            }
            
          } catch (error) {
            console.error(`  ❌ Error processing image ${i + 1}:`, error);
          }
        }
        
        console.log('═══════════════════════════════════════');
        console.log(`✅ Upload completed: ${uploadedCount}/${imageFiles.length} images`);
        console.log('═══════════════════════════════════════');
        
        if (uploadedCount > 0) {
          // ✅ CORRETTO: usa t() con interpolazione per il conteggio
          toast.success(t('messages.artwork_created_with_images', { count: uploadedCount }));
        } else {
          toast.warning(t('messages.artwork_created_no_images'));
        }
      } else if (imageFiles.length === 0) {
        console.log('ℹ️ No images to upload');
        toast.success(t('messages.artwork_created'));
      } else {
        toast.success(t('messages.artwork_updated'));
      }
      
    } catch (error) {
      console.error('❌ Error submitting form:', error);
      console.error('❌ Stack:', (error as Error).stack);
      toast.error((error as Error).message || t('errors.save_error'));
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-8">
      
      {/* ==================== SEZIONE 1: INFORMAZIONI BASE ==================== */}
      <div className="card">
        <button
          type="button"
          onClick={() => toggleSection('basic')}
          className="w-full flex items-center justify-between mb-4"
        >
          <h2 className="text-xl font-bold text-gray-900">
            📝 {t('components.artworks.artwork_form.section_basic')}
          </h2>
          {sectionsExpanded.basic ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
        </button>

        {sectionsExpanded.basic && (
          <div className="space-y-6">
            {/* Titolo */}
            <div>
              <label htmlFor="title" className="label">
                {t('components.artworks.artwork_form.label_title')} <span className="text-red-500">*</span>
              </label>
              <input
                id="title"
                type="text"
                {...register('title', { required: t('components.artworks.artwork_form.title_required') })}
                className="input"
                // ✅ CORRETTO: chiave con punti, non backslash
                placeholder={t('components.artworks.artwork_form.inserisci_il_titolo_opera')}
              />
              {errors.title && (
                <p className="mt-1 text-sm text-red-600">{errors.title.message}</p>
              )}
            </div>

            {/* Artista */}
            <div>
              <div className="flex items-center justify-between mb-1">
                {/* ✅ CORRETTO: chiave con punti */}
                <label className="label">{t('components.artworks.artwork_form.artista')}</label>
                <button
                  type="button"
                  onClick={() => {
                    setArtistInputMode(mode => mode === 'select' ? 'input' : 'select');
                    setCustomArtistName('');
                  }}
                  className="text-xs text-blue-600 hover:text-blue-700 flex items-center space-x-1"
                >
                  <Plus className="w-3 h-3" />
                  <span>
                    {artistInputMode === 'select' 
                      ? t('components.artworks.artwork_form.nuovo_artista') 
                      : t('components.artworks.artwork_form.seleziona_esistente')}
                  </span>
                </button>
              </div>

              {artistInputMode === 'select' ? (
                <>
                  {artistsLoading ? (
                    <div className="flex items-center space-x-2 text-gray-500 py-2">
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span className="text-sm">{t('components.artworks.artwork_form.caricamento_artisti')}</span>
                    </div>
                  ) : (
                    <select id="artist_id" {...register('artist_id')} className="input">
                      {/* ✅ CORRETTO: chiave con punti */}
                      <option value="">{t('components.artworks.artwork_form.nessun_artista')}</option>
                      {artists.map((artist: any) => (
                        <option key={artist.id} value={artist.id}>
                          {artist.name}
                        </option>
                      ))}
                    </select>
                  )}
                </>
              ) : (
                <input
                  type="text"
                  value={customArtistName}
                  onChange={(e) => setCustomArtistName(e.target.value)}
                  className="input"
                  placeholder={t('components.artworks.artwork_form.placeholder_artist_name')}
                />
              )}
            </div>

            {/* CATEGORIA */}
            <div>
              <label htmlFor="category" className="label">
                {t('components.artworks.artwork_form.label_category')} <span className="text-red-500">*</span>
              </label>
              <select
                id="category"
                {...register('category', { required: t('components.artworks.artwork_form.category_required') })}
                className="input"
              >
                <option value="">{t('components.artworks.artwork_form.select_category')}</option>
                {categories.map((cat) => (
                  <option key={cat.value} value={cat.value}>
                    {cat.label}
                  </option>
                ))}
                              </select>
              {errors.category && (
                <p className="mt-1 text-sm text-red-600">{errors.category.message}</p>
              )}
            </div>

            {/* SOTTOCATEGORIA */}
            {selectedCategory && availableSubcategories.length > 0 && (
              <div className="animate-fadeIn">
                <label htmlFor="subcategory" className="label">
                  {t('components.artworks.artwork_form.label_subcategory')}
                </label>
                <select id="subcategory" {...register('subcategory')} className="input">
                  <option value="">{t('components.artworks.artwork_form.select_subcategory')}</option>
                  {availableSubcategories.map((sub) => (
                    <option key={sub.value} value={sub.value}>
                      {sub.label}
                    </option>
                  ))}
                </select>
                <p className="mt-1 text-xs text-gray-500">
                  {t('components.artworks.artwork_form.subcategory_hint', { 
                    category: getCategoryByValue(selectedCategory, t)?.label.toLowerCase() 
                  })}
                </p>
              </div>
            )}

            {/* Anno e Tecnica */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="year" className="label">
                  {t('components.artworks.artwork_form.label_year')}
                </label>
                <input
                  id="year"
                  type="number"
                  {...register('year', {
                    // ✅ CORRETTO: chiavi con punti
                    min: { value: 1, message: t('components.artworks.artwork_form.anno_non_valido') },
                    max: { value: new Date().getFullYear(), message: t('components.artworks.artwork_form.anno_non_puo_essere') }
                  })}
                  className="input"
                  placeholder="Es: 2024"
                />
                {errors.year && <p className="mt-1 text-sm text-red-600">{errors.year.message}</p>}
              </div>

              <div>
                <label htmlFor="technique" className="label">
                  {t('components.artworks.artwork_form.label_technique')}
                </label>
                <input
                  id="technique"
                  type="text"
                  {...register('technique')}
                  className="input"
                  // ✅ CORRETTO: chiave con punti
                  placeholder={t('components.artworks.artwork_form.es_olio_su_tela')}
                />
              </div>
            </div>

            {/* Dimensioni */}
            <div>
              <label htmlFor="dimensions" className="label">
                {t('components.artworks.artwork_form.label_dimensions')}
              </label>
              <input
                id="dimensions"
                type="text"
                {...register('dimensions')}
                className="input"
                placeholder="Es: 77 x 53 cm"
              />
            </div>

            {/* Valore Stimato */}
            <div>
              <label htmlFor="estimated_value" className="label">
                {t('components.artworks.artwork_form.label_estimated_value')}
              </label>
              <input
                id="estimated_value"
                type="number"
                step="0.01"
                {...register('estimated_value', {
                  // ✅ CORRETTO: chiave con punti
                  min: { value: 0, message: t('components.artworks.artwork_form.il_valore_deve_essere') }
                })}
                className="input"
                placeholder="Es: 50000"
              />
            </div>

            {/* Descrizione */}
            <div>
              <label htmlFor="description" className="label">
                {t('components.artworks.artwork_form.label_description')}
              </label>
              <textarea
                id="description"
                {...register('description')}
                rows={4}
                className="input"
                placeholder={t('components.artworks.artwork_form.placeholder_description')}
              />
            </div>

            {/* GALLERIA IMMAGINI */}
            <div>
              <ImageGallery
                artworkId={artwork?.id}
                onImagesChange={handleImagesChange}
                maxImages={5}
              />
            </div>
          </div>
        )}
      </div>

      {/* ==================== SEZIONE 2: IDENTIFICAZIONE & DOCUMENTAZIONE ==================== */}
      <div className="card">
        <button
          type="button"
          onClick={() => toggleSection('identification')}
          className="w-full flex items-center justify-between mb-4"
        >
          <h2 className="text-xl font-bold text-gray-900">
            🔖 {t('components.artworks.artwork_form.section_identification')}
          </h2>
          {sectionsExpanded.identification ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
        </button>

        {sectionsExpanded.identification && (
          <div className="space-y-6">
            <div>
              <label htmlFor="catalog_number" className="label">
                {t('components.artworks.artwork_form.label_catalog_number')}
              </label>
              <input
                id="catalog_number"
                type="text"
                {...register('catalog_number')}
                className="input"
                placeholder="Es: KR-2024-001"
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="certificate_authenticity" className="label">
                  {t('components.artworks.artwork_form.label_certificate')}
                </label>
                <select
                  id="certificate_authenticity"
                  {...register('certificate_authenticity')}
                  className="input"
                >
                  <option value="">{t('components.artworks.artwork_form.non_specificato')}</option>
                  <option value="yes">{t('common.yes')}</option>
                  <option value="no">{t('common.no')}</option>
                </select>
              </div>

              {certificateAuth === 'yes' && (
                <div>
                  <label htmlFor="certificate_number" className="label">
                    {t('components.artworks.artwork_form.label_certificate_number')}
                  </label>
                  <input
                    id="certificate_number"
                    type="text"
                    {...register('certificate_number')}
                    className="input"
                    placeholder="Es: CERT-2024-12345"
                  />
                </div>
              )}
            </div>

            <div>
              <label htmlFor="artist_signature" className="label">
                {t('components.artworks.artwork_form.label_signature')}
              </label>
              <select
                id="artist_signature"
                {...register('artist_signature')}
                className="input"
              >
                <option value="">{t('components.artworks.artwork_form.non_specificato')}</option>
                <option value="yes">{t('common.yes')}</option>
                <option value="no">{t('common.no')}</option>
                <option value="unverifiable">{t('components.artworks.artwork_form.non_verificabile')}</option>
              </select>
            </div>

            <div>
              <label htmlFor="condition_state" className="label">
                {t('components.artworks.artwork_form.label_condition')}
              </label>
              <select
                id="condition_state"
                {...register('condition_state')}
                className="input"
              >
                <option value="">{t('components.artworks.artwork_form.non_specificato')}</option>
                <option value="excellent">{t('components.artworks.artwork_form.condition_excellent')}</option>
                <option value="good">{t('components.artworks.artwork_form.condition_good')}</option>
                <option value="fair">{t('components.artworks.artwork_form.condition_fair')}</option>
                <option value="needs_restoration">{t('components.artworks.artwork_form.da_restaurare')}</option>
              </select>
            </div>

            <div>
              <label htmlFor="edition_number" className="label">
                {t('components.artworks.artwork_form.label_edition')}
              </label>
              <input
                id="edition_number"
                type="text"
                {...register('edition_number')}
                className="input"
                placeholder="Es: 3/10"
              />
            </div>
          </div>
        )}
      </div>

      {/* ==================== SEZIONE 3: LOCALIZZAZIONE & LOGISTICA ==================== */}
      <div className="card">
        <button
          type="button"
          onClick={() => toggleSection('logistics')}
          className="w-full flex items-center justify-between mb-4"
        >
          <h2 className="text-xl font-bold text-gray-900">
            📍 {t('components.artworks.artwork_form.section_logistics')}
          </h2>
          {sectionsExpanded.logistics ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
        </button>

        {sectionsExpanded.logistics && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="current_location" className="label">
                  {t('components.artworks.artwork_form.label_location')}
                </label>
                <select
                  id="current_location"
                  {...register('current_location')}
                  className="input"
                >
                  <option value="">{t('components.artworks.artwork_form.non_specificato')}</option>
                  <option value="studio">{t('components.artworks.artwork_form.location_studio')}</option>
                  <option value="warehouse">{t('components.artworks.artwork_form.location_warehouse')}</option>
                  <option value="on_loan">{t('components.artworks.artwork_form.in_prestito')}</option>
                  <option value="on_display">{t('components.artworks.artwork_form.in_mostra')}</option>
                  <option value="other">{t('components.artworks.artwork_form.location_other')}</option>
                </select>
              </div>

              <div>
                <label htmlFor="location_details" className="label">
                  {t('components.artworks.artwork_form.label_location_details')}
                </label>
                <input
                  id="location_details"
                  type="text"
                  {...register('location_details')}
                  className="input"
                  placeholder={t('components.artworks.artwork_form.placeholder_location_details')}
                />
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label htmlFor="insurance_value" className="label">
                  {t('components.artworks.artwork_form.label_insurance_value')}
                </label>
                <input
                  id="insurance_value"
                  type="number"
                  step="0.01"
                  {...register('insurance_value')}
                  className="input"
                  placeholder="Es: 100000"
                />
              </div>

              <div>
                <label htmlFor="insurance_company" className="label">
                  {t('components.artworks.artwork_form.label_insurance_company')}
                </label>
                <input
                  id="insurance_company"
                  type="text"
                  {...register('insurance_company')}
                  className="input"
                  placeholder="Es: AXA Art"
                />
              </div>

              <div>
                <label htmlFor="insurance_expiry" className="label">
                  {t('components.artworks.artwork_form.label_insurance_expiry')}
                </label>
                <input
                  id="insurance_expiry"
                  type="date"
                  {...register('insurance_expiry')}
                  className="input"
                />
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="frame_included" className="label">
                  {t('components.artworks.artwork_form.label_frame')}
                </label>
                <select
                  id="frame_included"
                  {...register('frame_included')}
                  className="input"
                >
                  <option value="">{t('components.artworks.artwork_form.non_specificato')}</option>
                  <option value="yes">{t('common.yes')}</option>
                  <option value="no">{t('common.no')}</option>
                </select>
              </div>

              {frameIncluded === 'yes' && (
                <div>
                  <label htmlFor="frame_description" className="label">
                    {t('components.artworks.artwork_form.label_frame_description')}
                  </label>
                  <input
                    id="frame_description"
                    type="text"
                    {...register('frame_description')}
                    className="input"
                    placeholder={t('components.artworks.artwork_form.placeholder_frame_description')}
                  />
                </div>
              )}
            </div>
          </div>
        )}
      </div>

      {/* ==================== SEZIONE 4: MERCATO & TRANSAZIONI ==================== */}
      <div className="card">
        <button
          type="button"
          onClick={() => toggleSection('market')}
          className="w-full flex items-center justify-between mb-4"
        >
          <h2 className="text-xl font-bold text-gray-900">
            💰 {t('components.artworks.artwork_form.section_market')}
          </h2>
          {sectionsExpanded.market ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
        </button>

        {sectionsExpanded.market && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="purchase_price" className="label">
                  {t('components.artworks.artwork_form.label_purchase_price')}
                </label>
                <input
                  id="purchase_price"
                  type="number"
                  step="0.01"
                  {...register('purchase_price')}
                  className="input"
                  placeholder="Es: 30000"
                />
              </div>

              <div>
                <label htmlFor="purchase_date" className="label">
                  {t('components.artworks.artwork_form.label_purchase_date')}
                </label>
                <input
                  id="purchase_date"
                  type="date"
                  {...register('purchase_date')}
                  className="input"
                />
              </div>
            </div>

            <div>
              <label htmlFor="seller_gallery" className="label">
                {t('components.artworks.artwork_form.label_seller')}
              </label>
              <input
                id="seller_gallery"
                type="text"
                {...register('seller_gallery')}
                className="input"
                placeholder={t('components.artworks.artwork_form.placeholder_seller')}
              />
            </div>

            <div>
              <label htmlFor="provenance" className="label">
                {t('components.artworks.artwork_form.label_provenance')}
              </label>
              <textarea
                id="provenance"
                {...register('provenance')}
                rows={3}
                className="input"
                placeholder={t('components.artworks.artwork_form.placeholder_provenance')}
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="available_for_sale" className="label">
                  {t('components.artworks.artwork_form.label_available_for_sale')}
                </label>
                <select
                  id="available_for_sale"
                  {...register('available_for_sale')}
                  className="input"
                >
                  <option value="">{t('components.artworks.artwork_form.non_specificato')}</option>
                  <option value="yes">{t('common.yes')}</option>
                  <option value="no">{t('common.no')}</option>
                </select>
              </div>

              {availableForSale === 'yes' && (
                <div>
                  <label htmlFor="asking_price" className="label">
                    {t('components.artworks.artwork_form.label_asking_price')}
                  </label>
                  <input
                    id="asking_price"
                    type="number"
                    step="0.01"
                    {...register('asking_price')}
                    className="input"
                    placeholder="Es: 50000"
                  />
                </div>
              )}
            </div>
          </div>
        )}
      </div>

      {/* ==================== PULSANTI ==================== */}
      <div className="flex justify-end space-x-3 pt-4 border-t">
        <button
          type="submit"
          disabled={isSubmitting}
          className="btn btn-primary disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
        >
          {isSubmitting ? (
            <>
              <Loader2 className="w-4 h-4 animate-spin mr-2" />
              {t('components.artworks.artwork_form.saving')}
            </>
          ) : (
            <>
              {artwork 
                ? t('components.artworks.artwork_form.btn_update') 
                : t('components.artworks.artwork_form.btn_create')}
            </>
          )}
        </button>
      </div>
    </form>
  );
};