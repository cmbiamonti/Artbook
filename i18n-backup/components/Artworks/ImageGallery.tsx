...
export const ImageGallery: React.FC<ImageGalleryProps> = ({ 
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > availableSlots) {
      toast.error(`Puoi caricare massimo ${maxImages} immagini (${availableSlots} slot disponibili)`);
      return;
    }
    ...
    if (artworkId) {
      ...
      toast.success(`${validFiles.length} immagine/i aggiunta/e`);
    }
  ...
  const handleSetPrimary = async (index: number) => {
    if (!artworkId) {
      toast.info('Imposta come principale dopo aver creato l\'opera');
      return;
    }
  ...
  const handleDragEnd = async () => {
    ...
    try {
      const updatedOrder = images.map((img, idx) => ({
        id: img.id,
        order: idx
      }));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <label className="label">
          Galleria Immagini ({currentCount}/{maxImages})
        </label>
      ...
      {displayImages.length > 0 ? (
            ...
            {displayImages.map((img, index) => {
              ...
              return (
                <div
                  ...
                  draggable={artworkId ? true : false}
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center hover:border-blue-500 transition">
          ...
          <p className="text-gray-600 mb-2">Nessuna immagine caricata</p>
          <p className="text-sm text-gray-500">Clicca "Aggiungi Immagini" per caricare fino a {maxImages} immagini</p>
        </div>
      )}
    </div>
  );
